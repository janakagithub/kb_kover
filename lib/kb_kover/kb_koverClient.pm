package kb_kover::kb_koverClient;
...
sub new
{
    ...
    my $self = {
	client => kb_kover::kb_koverClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
=head2 amr_prediction

  $output = $obj->amr_prediction($params)
...
<pre>
$params is a kb_kover.FilterContigsParams
$output is a kb_kover.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a kb_kover.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a kb_kover.assembly_ref
...
# Authentication: required
	...
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function amr_prediction (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        ...
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to amr_prediction:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'amr_prediction');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "kb_kover.amr_prediction",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method amr_prediction",
...
sub version {
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method amr_prediction",
            status_line => $self->{client}->status_line,
            method_name => 'amr_prediction',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for kb_kover::kb_koverClient\n";
    }
    if ($sMajor == 0) {
        warn "kb_kover::kb_koverClient version is $svr_version. API subject to change.\n";
    }
}
...
package kb_kover::kb_koverClient::RpcClient;
